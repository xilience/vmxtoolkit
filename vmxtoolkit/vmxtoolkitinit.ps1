...
## Get OS type
Write-Output "Determining OS type..."
if ($env:windir)
{
...
elseif ($OS = uname)
{
    ...
    Switch ($OS)
    {
        "Darwin"
        {
            ...
            try    { $webrequestor = (get-command curl).Path }
            catch  { Write-Warning "curl not found"; return $false }
            
            try    { $GLobal:VMware_packer = (get-command 7za -ErrorAction Stop).Path }
            catch  { Write-Warning "7za not found, pleas install p7zip full"; return $false }
        ...
        'Linux'
        {
            ...
            try    { $VMware_Path = Split-Path -Parent (get-command vmware).Path }
            catch  { Write-Warning "VMware Path not found"; return $false }

            $Global:vmwarepath = $VMware_Path
            $VMware_BIN_Path = $VMware_Path
            try    { $Global:VMware_vdiskmanager = (get-command vmware-vdiskmanager).Path}
            catch  { Write-Warning "vmware-vdiskmanager not found"; return $false}

            try    { $GLobal:VMware_packer = (get-command 7za).Path }
            catch  { Write-Warning "7za not found, pleas install p7zip full" }

            try    { $Global:vmrun = (Get-Command vmrun).Path }
            catch  { Write-Warning "vmrun not found"; return $false }

            try    { $Global:VMware_OVFTool = (Get-Command ovftool).Path }
            catch  { Write-Warning "ovftool not found"; return $false}

            try    { $Global:mkisofs = (Get-Command mkisofs).Path }
            catch  { Write-Warning "mkisofs not found" ; return $false }
        ...
        default    { Write-Output "Sorry, rome was not build in one day"; return $false }
        'default'  { Write-Output "unknown linux OS"; return $false }
    }
}
else  { Write-Output "Error detecting OS"; return $false }
...
if (Test-Path $preferences_file)
{
    ...
    if ($defaultVMPath)
    {
        $VMX_default_Path = ((($defaultVMPath -replace "`"")-split "=")[-1]).TrimStart(" ")
        Write-Verbose "default vmpath from preferences is $VMX_default_Path"
        $defaultselection = "preferences"
    }
    else
    {
        Write-Verbose "no defaultVMPath in preferences"
    }
}
...
if (!$VMX_Path)
{
    if (!$VMX_default_Path)
    {
        ...
        try                               { $Global:vmxdir = Join-Path $HOME $VMX_BasePath; $defaultselection = "homedir" }
        catch                             { $Global:vmxdir = $PSScriptRoot;                 $defaultselection = "ScriptRoot"; Write-Warning "could not evaluate default Virtual machines home, using $PSScriptRoot"; Write-Verbose "using psscriptroot as vmxdir" }
    }
    else
    {
        if (Test-Path $VMX_default_Path)  { $Global:vmxdir = $VMX_default_Path }
        else                              { $Global:vmxdir = $PSScriptRoot }
    }
}
else                                      { $Global:vmxdir = $VMX_Path }
...
if (!$GLobal:VMware_packer) { Write-Warning "Please install 7za/p7zip, otherwise labbtools can not expand OS Masters" }

if ($OS_Version)  { Write-Output " ==>$OS_Version" } #-ForegroundColor Gray
else              { Write-Output "error Detecting OS"; return $false }
...
if ($VMX_Path)    { Write-Output " ==>using virtual machine directory from module load $Global:vmxdir" } #-ForegroundColor Gray
else              { Write-Output " ==>using virtual machine directory from $defaultselection`: $Global:vmxdir" } #-ForegroundColor Gray
